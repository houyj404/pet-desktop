// ══════════════════════════════════════════════════════════════
// 桌面小宠物 — db.rs
// SQLite 数据库: tasks, pet_state, settings
// ══════════════════════════════════════════════════════════════
...
impl DbState {
    /// Open (or create) the database at a fixed path next to the binary.
    pub fn new() -> Self {
        let db_path = db_path();
...
/// Cross-platform database path, stored next to the binary or in user data dir.
fn db_path() -> PathBuf {
    // Prefer next to the binary for portability
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            return dir.join("pet_data.db");
        }
    }
    PathBuf::from("pet_data.db")
}
